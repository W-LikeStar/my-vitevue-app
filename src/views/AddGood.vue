...
const state = reactive({
  ...
  category: { // 联动组件 props 属性
    lazy: true,
    lazyLoad(node, resolve) { // 懒加载分类方法在 lazyLoad 方法中，我们首先从后端接口获取指定分类的子分类列表，然后将返回的数据转换成一个包含 value 和 label 属性的节点数组
      const { level = 0, value } = node  //从 node 参数中获取当前节点的层级和值（即分类 ID） 分类 ID
      axios.get('/categories', {  //value表示分类的value，level表示这个value的第几个层级
        params: {
          pageNumber: 1,   //当用户点击某个节点时，Cascader 组件会根据该节点的 value 值调用 lazyLoad 方法去加载该节点的子节点数据。lazyLoad 方法需要向后端请求数据，并将返回的数据转化成节点对象的形式，然后通过 resolve 回调函数将节点数组返回给 Cascader 组件。组件会根据返回的节点数组自动生成下一级的选项列表。
          pageSize: 1000,
          categoryLevel: level + 1,  //表示请求的分类层级，即下一级子分类的层级。
          parentId: value || 0 //表示请求的分类的父分类 ID，即当前节点的值。
        }
      }).then(res => {
        const list = res.list
        const nodes = list.map(item => ({
          ...
          leaf: level > 1   //level三级联动，等于2的时候是叶子几点
        }))
        resolve(nodes)
      })
    }
...
onMounted(() => {
  ...
  instance.config.uploadImgHooks = {
    // 图片上传并返回了结果，想要自己把图片插入到编辑器中 这段代码是用于配置 wangEditor 在图片上传成功后的回调函数 uploadImgHooks
    // 例如服务器端返回的不是 { errno: 0, data: [...] } 这种格式，可使用 customInsert
    customInsert: function(insertImgFn, result) {   //customInsert 函数会在上传图片成功后执行。这个函数可以将上传成功的图片插入到编辑器中，
...
// 图片上传成功后的回调
const handleUrlSuccess = (val) => {//val表示上传成功后返回的信息对象，其中包含了上传成功后的图片地址。该函数的作用是将上传成功后的图片地址保存到Vue实例中的
